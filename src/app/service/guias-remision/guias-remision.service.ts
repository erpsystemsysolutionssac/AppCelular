...
export class GuiasRemisionService {
  ...
  listaGuiasPendientes(inicio: number, limite: number, texto: string, fechaInicio: string, fechaFinal: string, busqueda: string) {
    ...
    return this.http.post<any[]>(this.rutaApi + '/guia/app/v1/lista_documentos_pendientes/' + this.globalService.calcularNumeroRandomUrl(), body).toPromise()
  }

  consultarDetalleGuiaPendiente(codigoPuntoVenta: string, codigoMotivo: string, numero: string) {
    ...
    return this.http.post<any[]>(this.rutaApi + '/guia/app/v1/lista_detalle_pendientes/' + this.globalService.calcularNumeroRandomUrl(), body).toPromise()
  }
}
